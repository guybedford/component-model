...
name-chars          ::= l:<label>                                          => l
                      ...
                      | '[get]' r:<label> '.' g:<label>                    => [get]r.g
                      | '[set]' r:<label> '.' s:<label>                    => [set]r.s
label               ::= w:<word>                                           => w
                      | l:<label> '-' w:<word>                             => l-w
...
* Validation of `[get]` names requires the function to have one parameter and
  one unnamed result, with the parameter being `(param "self" (borrow $R))`.
* Validation of `[set]` names requires the function to have two parameters and
  no results, with the first parameter being `(param "self" (borrow $R))` and
  the second parameter being named `value`.
* Validation of `[method]`, `[static]`, `[get]` and `[set]` names ensures that
  that all field names are disjoint.
...
valtype       ::= i:<typeidx>                             => i
                | pvt:<primvaltype>                       => pvt
resourcetype  ::= 0x3f 0x01 0x7f f?:<funcidx>?            => (resource (rep i32) (dtor f)?)
...
canon    ::= 0x00 0x00 f:<core:funcidx> opts:<opts> ft:<typeidx> => (canon lift f opts type-index-space[ft])
           ...
           | 0x02 t:<typeidx>                                    => (canon resource.new t)
           | 0x03                                                => (canon resource.drop)
...
* Validation assigns `canon resource.new` the core type `(func (param i32) (result i32))`
  (because resource `rep` types are currently fixed to `i32`) and requires that
  `typeidx` refers to a concrete (i.e., not imported) resource type.
* Validation assigns `canon resource.drop` the core type `(func (param i32))`.
